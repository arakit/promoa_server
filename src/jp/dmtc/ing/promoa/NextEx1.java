 ...
 */
@WebServlet(name = "next_ex_1", urlPatterns = { "/next_ex_1" })
public class NextEx1 extends HttpServlet {
...
//			}
			...
			List<String> images_key_list = new ArrayList<String>();
			for(String k : md.image_urls.keySet()){
				images_key_list.add(k);
			}

			Collections.shuffle(images_key_list);

			for(String k : images_key_list){
				//String url = md.image_urls.get(k);
				md.select_images_id.add(k);
				if(md.select_images_id.size()==20) break;
			}



	        StringBuffer redURL = request.getRequestURL();
	        redURL.replace(redURL.lastIndexOf("/"), redURL.length(), "").append("/create");
	        System.out.println("redirect="+redURL.toString());
