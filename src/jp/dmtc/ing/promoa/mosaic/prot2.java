...
//  }
  ...
  public void paint(Graphics g, File src_image, BufferedImage[] tile){
    ...
    try {
    	...
    	int cw = width/ onecell;
    	int ch = height / onecell;

    	int celnum = 0;
    	for(int j=0;j<ch;j++){
	    	for(int i=0;i<cw;i++){
	    		g2.drawImage(tile[celnum%tile.length], i*onecell, j*onecell, null);
	    		celnum++;
	    	}
    	}

    	g2.drawImage(tile[0], 0, 0, null);
